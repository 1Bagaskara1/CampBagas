...
SELECT mahasiswa.*, jurusan.namajurusan FROM mahasiswa JOIN jurusan ON mahasiswa.idjurusan = jurusan.idjurusan GROUP BY NamaMahasiswa HAVING umur < 20;

SELECT mahasiswa.*, Kontrak.Nilai FROM Mahasiswa JOIN Kontrak ON Mahasiswa.NIM = Kontrak.NIM GROUP BY NamaMahasiswa HAVING Nilai <= 'B';
...
SELECT mahasiswa.*, Mata_Kuliah.NamaMatkul FROM mahasiswa JOIN Mata_Kuliah ON mahasiswa.KodeMatkul = Mata_Kuliah.KodeMatkul GROUP BY NamaMahasiswa HAVING NamaMatkul = 'Data Mining';
